...
-- doesn't pass them.
--
-- SAFETY DESIGN NOTE ON THE MONTHLY LIMIT: comms_check_send_allowed() is a
-- separate PRE-FLIGHT function, not folded into log_resident_message/
-- log_broadcast_message. It must run BEFORE the provider is called (from
-- app/actions/communications.ts), not after, and the log_* functions never
-- raise on the limit - only on bad input. If the limit check lived inside the
-- log function instead, a message that was actually sent (cost incurred,
-- provider call already made) could fail to be logged at all, silently
-- losing the record while the message still went out - worse than either
-- blocking the send or allowing it.
-- ============================================================================
...
-- Pre-flight check: must be called BEFORE the provider ever sends anything.
-- Raises on failure; callers catch this per-recipient in a broadcast loop
-- (marking that one recipient skipped) rather than letting it abort the
-- whole broadcast for everyone else.
create or replace function comms_check_send_allowed(
...
begin
  ...
  if v_category = 'emergency_notice' then
    return;
  end if;
  ...
  if v_count >= v_limit then
    raise exception
      'Monthly message limit reached for this resident (% sent this calendar month, limit %). The Owner can override this specific send if it is genuinely needed.',
      v_count, v_limit;
  end if;
end;
$$ language plpgsql stable security definer;
...
create or replace function log_resident_message(
  p_resident_id bigint,
  p_booking_id bigint,
  p_channel text,
...
begin
  ...
  -- Idempotent no-op: a double-click/refresh/retry carrying the same key
  -- returns the original row instead of creating a duplicate send record.
  if p_idempotency_key is not null then
    select message_id into v_existing from resident_messages where idempotency_key = p_idempotency_key;
  ...
  v_cost := comms_estimate_cost(p_channel, coalesce(v_category, 'custom'));

  insert into resident_messages (
    resident_id, booking_id, channel, category, template_id, message_body, recipient_mobile,
    status, provider, provider_message_id, failure_reason, estimated_cost, idempotency_key,
    sent_by, sent_at
  ) values (
    p_resident_id, p_booking_id, p_channel, v_category, p_template_id, p_message_body, p_recipient_mobile,
    p_status, p_provider, p_provider_message_id, p_failure_reason, v_cost, p_idempotency_key,
    auth.uid(), case when p_status in ('sent', 'delivered', 'read') then now() else null end
  )
